...
import { View, FlatList, Image, Text, StyleSheet, TouchableOpacity, Dimensions, Animated } from 'react-native';
        ...
        const fetchImages = async () => {
            ...
            try {
                if (activeType === 'liked') {
                    ...
                    setImages(res.images.filter((img) => {
        ...
        return (
            ...
            >
                <Animated.View style={[styles.cardContainer, { transform: [{ scale: scaleAnim }] }]}>
                    <Image source={{ uri: item.imageLink }} style={styles.cardImage} resizeMode="cover" />
                    <View style={styles.cardInfo}>
                        <Text style={styles.artTitle} numberOfLines={1}>{item.title}</Text>
                        <Text style={styles.artArtist} numberOfLines={1}>{item.artist?.name || 'Unknown Artist'}</Text>
                    </View>
                </Animated.View>
            </TouchableOpacity>
        );
