...
// Build a Cloudinary crop transformation URL from the original upload URL.
// cropData: { originX, originY, width, height } in original image pixels.
function buildCroppedUrl(originalUrl, cropData) {
  if (!cropData || !cropData.width || !cropData.height) return originalUrl;
  const { originX, originY, width, height } = cropData;
  const t = `c_crop,x_${originX},y_${originY},w_${width},h_${height}`;
...
const Upload = () => {
  ...
  const [image, setImage] = useState(null);         // full original picker asset
  const [cropData, setCropData] = useState(null);   // { originX, originY, width, height } pixels
  ...
  const selectImage = async () => {
    ...
    // allowsEditing: false — we handle crop ourselves so the full original is preserved
    ...
    if (!picker.canceled) {
      ...
      // Convert to JPEG at full resolution — no resize, no crop.
      // Required because iPhones return HEIC by default, which browsers and the
      // admin panel cannot display. Dimensions are unchanged by this step.
      ...
      setImage({ ...selected, uri: jpeg.uri });
      setCropData(null);
      setShowCropSelector(true);
    }
  };

  const handleCropConfirmed = (pixelCrop) => {
    setCropData(pixelCrop);
    setShowCropSelector(false);
  };

  const handleCropCancelled = () => {
    // Keep the image but with no crop (full image)
    setCropData(null);
    setShowCropSelector(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1) Upload the ORIGINAL image to Cloudinary (no crop, no resize)
      const formData = new FormData();
      formData.append("file", {
        uri: image.uri,
        name: `${title}.jpg`,
        type: "image/jpeg",  // always JPEG — converted in selectImage
      });
      ...
      // 2) Build the display URL — Cloudinary crop transformation applied on-the-fly
      const displayUrl = buildCroppedUrl(originalUrl, cropData);
      ...
      const payload = {
        ...
        name: title,
        imageLink: displayUrl,           // cropped view (or full original if no crop)
        originalImageLink: originalUrl,  // always the untouched original
        price: priceVal,
        description,
        category,
        stage: "review",
  ...
  return (
    <ScreenTemplate>
      ...
      <Modal
        ...
        onRequestClose={handleCropCancelled}
      >
        {image ? (
          <CropSelector
            imageUri={image.uri}
...
const styles = StyleSheet.create({
  ...
  imageBox: {
    ...
    marginBottom: 8, height: 200, justifyContent: "center", alignItems: "center",
  ...
  cropStatusText: { fontSize: 12, color: "#555" },
  recropLink: { fontSize: 12, color: "#007bff", fontWeight: "700" },

  button: {
    backgroundColor: "#007bff", paddingVertical: 14, borderRadius: 10,
  ...
  modalTitle: { fontSize: 18, fontWeight: "800", color: "#0F172A", textAlign: "center", marginBottom: 6 },
  modalSub: { fontSize: 12, color: "#6B7280", textAlign: "center", marginBottom: 12 },

  diagramWrap: { alignItems: "center", marginBottom: 12 },
  canvas: { width: 260, height: 180, backgroundColor: "#F9FAFB", borderRadius: 10, borderWidth: 1, borderColor: "#E5E7EB", justifyContent: "center", alignItems: "center" },
  rect: { width: 160, height: 100, backgroundColor: "#fff", borderWidth: 2, borderColor: "#9CA3AF", borderRadius: 6, justifyContent: "center", alignItems: "center" },
  arrowH: { position: "absolute", width: 160, height: 16, flexDirection: "row", alignItems: "center", gap: 8, marginTop: 6 },
  ...
  depthBadge: { position: "absolute", right: -12, bottom: -14, backgroundColor: "#EEF2FF", borderColor: "#C7D2FE", borderWidth: 1, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999 },
  ...
  closeBtn: { alignSelf: "center", backgroundColor: "#111827", paddingVertical: 10, paddingHorizontal: 20, borderRadius: 999 },
  closeBtnText: { color: "#fff", fontWeight: "800", letterSpacing: 0.4 },

  loadingOverlay: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, justifyContent: "center", alignItems: "center", backgroundColor: "rgba(255,255,255,0.7)", zIndex: 999 },
});
